...
struct PageViewControllerWrapper: UIViewControllerRepresentable {
    ...
    func makeHostingController(for pageID: String) -> UIViewController? {
        ...
        // figures out if this is a past page and which choice was made
        let isCurrentPage = (pageID == viewModel.currentPageID)
        var selectedChoiceDestination: String? = nil
        if !isCurrentPage,
           let idx = viewModel.history.lastIndex(of: pageID),
           idx + 1 < viewModel.history.count {
            selectedChoiceDestination = viewModel.history[idx + 1]
        }
        ...
        let host = UIHostingController(rootView: StoryPageView(
            ...
            onNameCreature: isCurrentPage && page.showNameField ? { name in
                self.viewModel.nameCreature(name)
            } : nil,
            selectedChoiceDestination: selectedChoiceDestination
        ))
    ...
    class Coordinator: NSObject, UIPageViewControllerDataSource, UIPageViewControllerDelegate {
        ...
        func pageViewController(_ pvc: UIPageViewController,
            ...
            // allow forward swipe only through already-visited history
            guard let pageID = vc.view.accessibilityIdentifier,
                  let index = wrapper.viewModel.history.lastIndex(of: pageID),
                  index + 1 < wrapper.viewModel.history.count else { return nil }
            return wrapper.makeHostingController(for: wrapper.viewModel.history[index + 1])
        }
    }
}
